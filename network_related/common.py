...
def conv_act_layer_norelu(from_layer, name, num_filter, kernel=(1,1), pad=(0,0), \
    stride=(1,1), act_type="relu", use_batchnorm=False): # use_batchnorm=False
    
    conv = mx.symbol.Convolution(data=from_layer, kernel=kernel, pad=pad, \
        stride=stride, num_filter=num_filter, no_bias=True, name="{}_conv".format(name), num_group=1)
    #if use_batchnorm:
...
def conv_act_layer(from_layer, name, num_filter, kernel=(1,1), pad=(0,0), \
    ...
    conv = mx.symbol.Convolution(data=from_layer, kernel=kernel, pad=pad, \
        stride=stride, num_filter=num_filter, no_bias=True, name="{}_conv".format(name), num_group=1)
    if use_batchnorm:
        conv = mx.symbol.BatchNorm(data=conv, fix_gamma=False, momentum=0.9, eps=2e-5, name="{}_bn".format(name))
    ...
    for k, params in enumerate(zip(from_layers, num_filters, strides, pads)):
        ...
        if from_layer.strip():
            ...
            conv_3x3 = conv_act_layer(conv_1x1, 'multi_feat_%d_conv_3x3' % (k),
                num_filter, kernel=(3, 3), pad=(p, p), stride=(s, s), act_type='relu', use_batchnorm=True)
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        if normalization[k] > 0:
            ...
            scale = mx.symbol.Variable(name="{}_scale".format(from_name),
                shape=(1, num_channels.pop(0), 1, 1), #num_channels.pop(0)
        ...
        if k > -1:
            ...
            cls_pred = mx.symbol.Convolution(data=from_layer, bias=bias, kernel=(5,5),
                pad=(2,2), num_filter=num_cls_pred, \
                name="{}_cls_pred_conv".format(from_name))
